...
function _setEnvVars(){
  case $_network in
      Mainnet)
        ...
        REACT_APP_RPC_URL="http://$HOST:8545" 
        echo "REACT_APP_RPC_URL=$REACT_APP_RPC_URL" >> "${LAUNCHPAD_SOURCE_DIR}/.env"
        # REACT_APP_BEACONCHAIN_URL="http://$HOST:5052" 
      ...
      Hoodi)
        ...
        REACT_APP_RPC_URL="http://$HOST:8545" 
        echo "REACT_APP_RPC_URL=$REACT_APP_RPC_URL" >> "${LAUNCHPAD_SOURCE_DIR}/.env"
        # Can't use local node for BEACONCHAIN_URL because it relies on public apis not available on the node
        # REACT_APP_BEACONCHAIN_URL="http://$HOST:5052" 
        # echo "REACT_APP_BEACONCHAIN_URL=$REACT_APP_BEACONCHAIN_URL" >> "${LAUNCHPAD_SOURCE_DIR}/.env"
      ;;
        "Network Syncing")
